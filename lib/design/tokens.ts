...
export type DesignTokens = {
  ...
  shadow: { 1: string; 2: string };
  font: { sans: string; mono: string };
};

/* --------------------------
 *  Defaults (match globals.css)
 * -------------------------- */
...
/** Safe on server: falls back to defaults */
...
/** React hook that updates when `data-theme` changes */
export function useDesignTokens(): DesignTokens {
  const [tokens, setTokens] = React.useState<DesignTokens>(() => readTokens());
...
export function chartPalette(): string[] {
  ...
  return [b, withAlpha(b, 0.85), withAlpha(b, 0.7), withAlpha(b, 0.55), withAlpha(b, 0.4)];
}

function withAlpha(color: string, a: number) {
  if (color.trim().startsWith("oklch(")) return color.replace(/\)$/, ` / ${a})`);
  if (color.trim().startsWith("rgb(")) return color.replace(/\)$/, ` / ${a})`);
